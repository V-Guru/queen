...
package com.wozart.aura.ui.customview.rangebar;
...
public class Bar {
     ...
     * @param tickStart    the number start
     */
    ...
    public void drawTicks(Canvas canvas) {
        ...
        for (int i = 0; i < mNumSegments; i++) {
            ...
            drawNumber((int)mTickStart + i, x, canvas);
        }

        drawNumber(mNumSegments + (int)mTickStart, mRightX, canvas);
    ...
    private void drawNumber(int i, float x, Canvas canvas) {
        Paint paint = new Paint();

        paint.setColor(Color.BLACK);
        paint.setTextSize(30);
        String value = String.valueOf(i);
        canvas.drawText(String.valueOf(i), x - value.length() * 6, mY + 45, paint);
    }
}
